...
// ローテーション対策: 直近(全ジャンル数-1)本以内に使われたジャンルは今回の抽選から除外する。
// こうすることで、一度伸びたジャンルの重みがどれだけ大きくても「必ず一巡してから次の巡目に
// 入る」ことが保証される。実績が無い/一度も使われていないジャンルは当然この除外対象に入らない
// ため、自動的に優先的な候補になる(コールドスタート対策も兼ねる)。
...
const recentlyUsedGenres = new Set(usedTopics.slice(-ROTATION_WINDOW).map((t) => t.category));
...
- 【重要・視聴維持率に直結】基礎セグメントで内容を説明し切ってしまわないこと。フックでは「注意点」の核心(最も意外性のある内容)を全部明かさず「実は〜」「知らないと損する」等で軽く予告するにとどめ、基礎→仕組みでは前提知識のみを淡々と積み上げ、rank1(注意点)で初めて核心を明かす「引っ張る」構成にする。基礎の時点で全てを説明し切ると、最後まで見る理由がなくなり離脱の原因になるため厳禁
- rank3セグメントを「基礎」(用語の定義・全体像、最小限にとどめる)、rank2セグメントを「仕組み」(具体的にどう機能するか)、rank1セグメントを「注意点」(誤解しやすい点・気をつけるべき点。動画の中で最も意外性のある核心部分にすること)として扱うこと`,
    titleInstructions:
...
- tagsには、検索ボリュームが見込める一般的なキーワード(例:投資初心者、経済雑学、資産運用)を必ず2つ以上と、その動画固有の具体的キーワードを組み合わせて含めること。特にテクニカル分析等の専門用語ジャンルでは、専門用語だけで固めず(例:「RSI」「オシレーター」「標準偏差」のような専門用語のみのタグ構成は不可)、専門知識がない視聴者も検索・興味を持つ一般的な言い回しを必ず混ぜること。tagsは検索ボリュームが大きいと想定される順に並べること(先頭ほど需要が大きいものにする)
