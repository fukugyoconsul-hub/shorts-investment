...
# SEO対策のルール(重要)
- titleには、検索されやすい具体的なキーワード(専門用語・数字)を必ず含めること
- tagsには、検索ボリュームが見込める一般的なキーワード(例:FX,投資初心者)と、その動画固有の具体的キーワード(専門用語)を両方含めること。tagsの中の最初の数個は概要欄のハッシュタグとしても使われるため、この動画で最も検索されやすい具体的なキーワードを先頭寄りに並べること
