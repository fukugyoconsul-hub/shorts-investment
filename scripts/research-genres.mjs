...
# briefを書く上での注意(重要)
- 同じジャンルは繰り返し選ばれて何度も動画が作られる。format:rankingで、かつ「〜の記録」「歴史的な出来事の紹介」のような客観的な事実・記録を列挙するテーマの場合、briefに以下のような指示を必ず含めること:「タイトルは『〜ベスト3』のような『これが上位3つ』と限定する表現は避け、『〜3選』のように数ある中から3つを選んだことが伝わる表現にする(別の回の動画と別の3つを紹介しても矛盾しないようにするため)」
