...
const TARGET_GENRES = new Set(["株式", "コモディティ", "経済史"]);
...
for (const entry of usedTopics) {
  if (!entry.videoId || !TARGET_GENRES.has(entry.category)) continue;
  if (!entry.title.includes("ベスト3")) continue;
  const newTitle = entry.title.replace(/ベスト3/g, "3選");
